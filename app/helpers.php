...
if (! function_exists('getSuperAdminSettingValue')) {
     ...
     */
    function getSuperAdminSettingValue(): Illuminate\Database\Eloquent\Collection|SuperAdminSetting
    {
        ...
        if (empty($appLogo)) {
            if (isAuth() && Auth::user()->hasRole(Role::ROLE_CLIENT)) {
                ...
                if ($setting->value == $img) {
                    ...
                    return !empty($userSetting['app_logo']) ? $userSetting['app_logo'] : $setting->value;
                }
            } else {
                if (isAuth() && Auth::user()->hasRole(Role::ROLE_ADMIN)) {
                    /** @var Setting $setting */
        ...
        if (empty($appLogo)) {
            if (isAuth() && Auth::user()->hasRole(Role::ROLE_CLIENT)) {
                ...
                if ($setting->value == $img) {
                    ...
                    return !empty($userSetting['favicon_icon']) ? $userSetting['favicon_icon'] : $setting->value;
                }
            } else {
                if (isAuth() && Auth::user()->hasRole(Role::ROLE_ADMIN)) {
                    /** @var Setting $setting */
